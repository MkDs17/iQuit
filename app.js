...
function init() {
  ...
  if (windowSize > 768 && windowSize <= 1400) {
    divPerPage = 3;
  }
  if (windowSize <= 768) {
    divPerPage = 1;
...
function getQuitTime() {
  ...
  const fallback = new Date("2025-01-09T21:25:00");
  try {
    const quitDate = new Date(
      params.has("date") ? params.get("date") : fallback
    );
    return quitDate;
  } catch (e) {
    return fallback;
...
function calculateTimeBtwTwoDates() {
  ...
  $("#quiterSince")
    .empty()
    .html("<span>since " + quiterSinceString + "</span>");
}

function loadDynamicInfos() {
  // Base value
  const cigarettesByDay = params.has("cigs") ? params.get("cigs") : 10;
  priceOneCig = 0.625;

  // let nonSmokinCigarettes = cigarettesByDay * diffDays
  unsmokedCigarettesLiveValue =
    cigarettesByDay * (diffTime / 1000 / 60 / 60 / 24);
  unsmokedCigarettesLiveValueRounded = (
    cigarettesByDay *
    (diffTime / 1000 / 60 / 60 / 24)
  ).toFixed(5);
  unsmokedCigarettesDecimals = String(
    (unsmokedCigarettesLiveValueRounded + "").split(".")[1]
  ).slice(0, 2);
  unsmokedCigarettes = Math.floor(
    cigarettesByDay * (diffTime / 1000 / 60 / 60 / 24)
  );
  unsmokedCigarettesToDisplay = Math.floor(
    cigarettesByDay * (diffTime / 1000 / 60 / 60 / 24)
  ).toLocaleString();
  ...
  successBadge =
    '<span class="absolute -top-3 right-0"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-12 h-12 text-sky-500"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></span>';
  ...
  $("#unsmokedCigarettesLiveValue")
    .empty()
    .html("unsmokedCigarettesLiveValue: " + unsmokedCigarettesLiveValue);
  $("#unsmokedCigarettesLiveValueRounded").html(
    "unsmokedCigarettesLiveValueRounded: " + unsmokedCigarettesLiveValueRounded
  );
  $("#unsmokedCigarettesLiveValueDecimals").html(
    "unsmokedCigarettesLiveValueDecimals: " + unsmokedCigarettesDecimals
  );
  $("#unsmokedCigarettes").html(
    "unsmokedCigarettes " + unsmokedCigarettesToDisplay
  );
  ...
  $("#fifteenYearsPourcentageStyle").css(
    "width",
    fifteenYearsPourcentage + "%"
  );
...
function loadGauge() {
  const opts = {
    ...
    showValue: true,
    label: function () {
      return unsmokedCigarettesToDisplay;
